...
the shear modulus $G$ and Poisson's ratio $\nu$ as coefficients (one per type),
and the `bond_style` reads $G$ from the improper style and scans the
tetrahedral topology to compute each bond's stiffness automatically. 
No bond coefficients must be provided. However, both bond (edge) and
improper (tetrahedron) topologies must be defined in the LAMMPS data file.
...
data/
    ...
    lam/            # generated LAMMPS data files (not tracked)
    cube.py         # example: generate .lam files for a unit cube
tests/
    in.hydro-comp   # hydrostatic compression
    in.ztension     # uniaxial tension
```
...
Copy the `.h`/`.cpp` files from `lammps/` to your LAMMPS `src/` folder,
then rebuild. No additional packages are required.

#### LAMMPS documentation

Copy the `.rst` files from `lammps/` to your
...
See `data/cube.py` for a complete example that generates data files at
multiple mesh refinement levels and prints CFL timestep estimates.
...
(Verify!) By default, each `Part` in `model.py` has the same improper type.

## Validation

Validation scripts are found in the `tests/` directory. 

**(TO DO) Hydrostatic compression** (`tests/in.hydro-comp`): All atoms are
displaced affinely by a small volumetric strain increment, then the
interior is relaxed with `fix nve` while boundary atoms are constrained
in their face-normal direction only (frictionless walls via
`fix setforce`). The bulk modulus is extracted from the slope of mean
pressure vs. volumetric strain: $K = -P / \varepsilon_v$, where $P$ is
computed from the reaction forces on the six faces and
